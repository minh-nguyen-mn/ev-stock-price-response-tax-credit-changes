...
*Find event date
*March 31, 2023 day = 231 (announcement)

sort date
egen day = group(date)

*March 31, 2023 day = 231 (announcement)
gen event_time = day - 231
...
* DO THIS 
*gen market_return = ((adjclose - l.adjclose)/l.adjclose)*100 using "GSPC.dta"
...
* We use CAPM to find daily market return because FF3FM data isn't recent enough
...
// title("Abnormal Returns by Brand (Announcement)") xtitle("Days Relative to Announcement") ytitle("") yline(0)
...
esttab model1 model2 model3 using announcement.rtf, label title() replace
...
// (connect ccar event_time if tic =="RIVN"), ///
// legend(label(1 "Ford") label(2 "Nissan") label(3 "Tesla") label(4 "Rivian") ) ///
// title("CARS by Brand Starting 10 Days Prior to Announcement") xtitle("Days Relative to Announcement") ytitle("") yline(0)
...
title(Average CARs and 95% CI Bands (Announcement)) xtitle("Days Relative to Announcement") 
...
legend(order(2 1) label(1 "95% CI Bands") label(2 "CAARs from t = -10 to t") ) ///
xtitle("Days Relative to Announcement") title("CARS of Ford and Nissan Relative to Rivian (Announcement)")
